...
Implement this story incrementally, following Clean Architecture and reusing existing models/layers to reduce risk and regressions, including pre-authorized onboarding and first-login auth linking.

## 2. Layer impact
- UI: Screen, state, and user action updates required by HU-010.
- Domain: Business rules and validations tied to linked RFs, including unauthorized mode and first authorized access.
...
- Ensure UI states match weekly rules, permissions, and unauthorized-restricted mode.
...
### Functions/Backend
- Implement/adjust data rules for email pre-authorization, first-login `authUid` linking, and unauthorized blocking.
- Keep compatibility with the proposed MVP Firestore structure.

## 4. Test strategy
- Unit tests for impacted business rules.
- Integration tests for relevant repository/data paths, including pre-authorized and unauthorized login flows.
