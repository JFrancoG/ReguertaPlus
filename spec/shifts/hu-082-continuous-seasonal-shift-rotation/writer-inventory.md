...
| `firestore-client-shift-planning-requests` | Android/iOS still submit the four-field legacy request directly. The local Phase 1 candidate denies all client access to this root, while strict Rules admit only exact admin-created v2 requests and admin read-back. The new authenticated read-only context now supplies the private open write epoch/active revision without exposing the state document, but neither client consumes it yet | `shiftPlanningRequests` | Migrate both clients to resolve context and create the exact two-subplan v2 request. During no-gap activation, deny new creates before causal capture and drain the accepted request set before changing maintenance state |
