...
| `firestore-client-shift-planning-requests` | Android/iOS now resolve the authenticated minimal context and transactionally create or acknowledge one exact combined schema-v2 `preview` request with explicit delivery/market target seasons, stable request/bundle identity, and the current write epoch/active revision. They reject malformed context and incompatible replay, expose no stage/activate authority, and continue observing backend-owned lifecycle/candidate state. The local Phase 1 candidate still denies this root; strict Rules admit the exact admin create/read contract | `shiftPlanningRequests` | During no-gap activation, deploy/read back the strict contract, deny new creates before causal capture, and drain the accepted request set before changing maintenance state |
