...
| `https-sync-shifts-from-google-sheets` | `syncShiftsFromGoogleSheets`; each upsert and stale imported-shift deletion now transactionally reads the exact shift notification fence and rechecks stale-delete ownership | Sheets to `shifts` | Keep the dispatch fence, but disable ingress before causal capture and drain; the multi-shift import remains partially applicable and HU-083 replaces this non-atomic writer |
...
| `https-transition-shift-swap` | Android/iOS HTTP clients and `transitionShiftSwap`; apply now transactionally reads every changed shift's notification-resource fence | `shiftSwapRequests`, assignments, helpers and shift status | Keep the dispatch fence, disable ingress before causal capture and drain accepted requests; then add the maintenance epoch/active-revision check |
| `trigger-on-shift-planning-request-created` | Legacy trigger; each `persistPlannedShifts` upsert now transactionally reads the exact shift notification fence after the workbook write | request, workbook and `shifts` | Keep the dispatch fence, drain only the causally captured accepted set, then disable the delivery; its workbook-first flow is still non-atomic |
