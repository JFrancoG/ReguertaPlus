...
| `firestore-client-delivery-calendar` | Android/iOS `FirestoreDeliveryCalendarRepository` still upsert/delete directly; the local `resolveDeliveryCalendarMutationContext` + `transitionDeliveryCalendarOverride` backend pair now provides exact authority/override CAS, active-admin revalidation, server-derived values, and idempotent private receipts | `deliveryCalendar` | Migrate both clients to the command, prove stale/offline queue rejection, then apply `rules-deny` before causal capture |
