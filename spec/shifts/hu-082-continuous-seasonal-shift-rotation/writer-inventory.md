...
| `firestore-client-shift-planning-requests` | Android/iOS still submit the four-field legacy request directly. The local Phase 1 candidate denies all client access to this root, while strict Rules admit only exact admin-created v2 requests and admin read-back; therefore neither current client can submit under the future strict contract | `shiftPlanningRequests` | First provide an authenticated server context for the private epoch/active lineage and migrate both clients to the exact two-subplan v2 request. During no-gap activation, deny new creates before causal capture and drain the accepted request set before changing maintenance state |
