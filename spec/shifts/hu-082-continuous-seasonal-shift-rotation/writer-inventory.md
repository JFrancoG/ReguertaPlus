...
`hu082-affected-writers-v2`. Its canonical digest is part of every intake-barrier
...
| `firestore-client-shift-planning-requests` | Android/iOS resolve the authenticated minimal context and transactionally create or acknowledge one exact combined schema-v2 `preview` request with explicit delivery/market target seasons, stable request/bundle identity, and the current write epoch/active revision. From their own completed preview, an authorized admin may submit one new `stage` request bound to that exact source request, bundle revision, and bundle digest; a changed, foreign, malformed, or self-referential binding fails closed. The clients expose no activate authority, discover only their administrator’s v2 requests, pin an in-flight request by document ID and recover source/candidate listeners with cancelable backoff. The query requires the composite index declared in `firestore.indexes.json`, READY before client enablement. The local Phase 1 candidate still denies this root; strict Rules admit admin reads plus exact `preview` creates and `stage` creates bound to an existing backend-completed preview owned by the same admin with matching bundle lineage. Every client `activate`, update, or delete remains denied. Backend activation uses Admin SDK authority outside Rules | `shiftPlanningRequests` | During no-gap activation, deploy/read back the strict contract, deny new preview/stage creates before causal capture, and drain the accepted request set before changing maintenance state |
...
| `trigger-on-shift-planning-request-created` | `onShiftPlanningRequestCreated`, legacy only with v2 excluded and retries disabled; immediately before its workbook write it captures the exact open planning authority, then each `persistPlannedShifts` upsert and its final notification transaction revalidate it; shift upserts also read the exact notification fence | request, workbook, `shifts` and notification | Keep both guards, drain only the causally captured accepted set, then disable the delivery; authority drift stops remaining Firestore effects, while its workbook-first flow stays non-atomic and requires HU-083 replacement |
| `trigger-on-versioned-shift-planning-request-created` | `onVersionedShiftPlanningRequestCreated`; schema-v2-only idempotent lifecycle/CAS with retry enabled, including transient authorization/source lookup errors and busy leases | requests, private planning state, atomic public activation and held side effects | Deploy only alongside the legacy update that excludes v2; include accepted requests, live/expired worker leases and pending Eventarc redeliveries in causal capture/drain before disabling delivery. Retry configuration alone does not prove queue closure |
...
- Epoch/revision migration of the remaining mobile and backend writers; the
  combined mobile planning `preview` and exact completed-preview `stage` ingress
  already carry their required authority and immutable lineage.
