...
class ReguertaFirebaseMessagingService : FirebaseMessagingService() {
    ...
    override fun onNewToken(token: String) {
        ...
        val authorizedContext = preferences.getAuthorizedSessionContext()
        if (authorizedContext == null) {
            Log.w(TAG, "Token received but no authorized session context is cached yet")
            return
        }
        val nowMillis = System.currentTimeMillis()
        val repository = FirestoreDeviceRegistrationRepository(firestore = FirebaseFirestore.getInstance())
        CoroutineScope(Dispatchers.IO).launch {
            Log.d(TAG, "Uploading refreshed FCM token to Firestore for member=${authorizedContext.memberId}")
            try {
                repository.registerDevice(
                    ...
                    isSessionCurrent = {
                        preferences.getAuthorizedSessionContext() == authorizedContext
                    },
                )
            } catch (_: CancellationException) {
                Log.d(TAG, "Skipping FCM token upload for a superseded authorized session")
            } catch (error: Exception) {
                Log.e(TAG, "Failed to upload refreshed FCM token", error)
            }
        }
    }
