...
- If `products.isEcoBasket == true`, `products.ecoBasketOption` is required in (`pickup`, `no_pickup`).
- `ecoBasketOption = no_pickup` means the commitment is paid but basket pickup is skipped.
- All active eco-basket products must share the same `price`, regardless of `ecoBasketOption` (`pickup`/`no_pickup`) and parity producer (`even`/`odd`).
