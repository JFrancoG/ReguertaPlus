...
- RF-NOTI-05 System must store user devices in `users/{userId}/devices/{deviceId}`, persist the latest platform registration credential (`fcmToken` for legacy/iOS or `firebaseInstallationId` for Android), and keep `users.lastDeviceId` updated with the latest active device.
